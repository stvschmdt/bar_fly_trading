#!/usr/bin/env python
"""
Runner for Regression Momentum Strategy.
...
Usage:
    ...
    # Full backtest (technical only, no ML — will warn about missing columns)
    python strategies/run_regression_momentum.py \
...
class RegressionMomentumRunner(BaseRunner):
    STRATEGY_NAME = "regression_momentum"
    EMAIL_TAG = "[REGRESSION]"

    @classmethod
    def add_strategy_args(cls, parser):
        parser.add_argument("--max-hold-days", type=int, default=13,
                            help="Max days to hold a position (default: 13)")

    def create_strategy(self, account, symbols, args, data=None):
        return RegressionMomentumStrategy(
            account=account,
            symbols=symbols,
            data=data,
